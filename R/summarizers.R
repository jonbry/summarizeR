...
group_stats <- function(df, var, num_stats) {
  ...
  quart <- df |>
    mutate(
      (!!sym(var_name)) := case_wen(
        between(!!sym(var), num_stats[[1, 2]], num_stats[[2, 2]]) ~
          paste("Q1 <", num_stats[[2, 2]]),
        between(!!sym(var), num_stats[[2, 2]], num_stats[[3, 2]]) ~
          paste("Q2 between ", num_stats[[2, 2]], "and", num_stats[[3, 2]]),
        between(!!sym(var), num_stats[[3, 2]], num_stats[[4, 2]]) ~
          paste("Q3 between", num_stats[[3, 2]], "and", num_stats[[4, 2]]),
        between(!!sym(var), num_stats[[4, 2]], num_stats[[5, 2]]) ~
          paste("Q4 bteween", num_stats[[4, 2]], "and", num_stats[[5, 2]]),
        .default = "missing"
      )
    )
}
